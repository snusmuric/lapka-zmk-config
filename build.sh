...
west build -s app -d "$SCRIPT_DIR/build/left"  -b nice_nano_v2 -- -DSHIELD=lapka_left  -DZMK_CONFIG="$SCRIPT_DIR/config" -DZMK_EXTRA_MODULES="$SCRIPT_DIR"
echo "  -> $SCRIPT_DIR/build/left/zephyr/zmk.uf2"

echo ""
echo "Building right half..."
west build -s app -d "$SCRIPT_DIR/build/right" -b nice_nano_v2 -- -DSHIELD=lapka_right -DZMK_CONFIG="$SCRIPT_DIR/config" -DZMK_EXTRA_MODULES="$SCRIPT_DIR"
echo "  -> $SCRIPT_DIR/build/right/zephyr/zmk.uf2"

echo ""
echo "Building settings reset..."
west build -s app -d "$SCRIPT_DIR/build/reset" -b nice_nano_v2 -- -DSHIELD=settings_reset -DZMK_CONFIG="$SCRIPT_DIR/config" -DZMK_EXTRA_MODULES="$SCRIPT_DIR"
